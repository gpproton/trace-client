...
<template>
    <identity-form>
        ...
        <template #sub-title>{{ $t('auth.otpHasBeenSent') }}
            <span class="text-overline">+234XXX123XXXX</span>
        </template>

        <div class="full-width q-my-lg row justify-center">
            <q-input v-for="(verifyItem, verifyIndex) in 4" :ref="(el: any) => {
                otpInput[verifyIndex] = el;
            }
                " maxlength="1" v-model="otpInputValue[verifyIndex]" :key="verifyIndex"
                :rules="[(val) => val.length <= 1]" type="text" standout no-error-icon
                class="border-radius-sm q-mx-sm verify-input" @keyup="handleKey(verifyIndex)" />
        </div>

        <q-separator class="q-mb-lg q-mx-lg" />

        <div class="q-my-md">
            <q-btn label="Verify" color="action" size="lg" no-caps class="full-width border-radius-sm text-weight-medium"
                @click="verifyToken" />
        </div>
        ...
        <template #footer>
            <div class="full-width row items-center justify-center q-pa-xs q-px-md">
                <div>
                    ...
                    <router-link class="identity-link identity-text text-weight-regular" :to="{ name: 'auth.sign-in' }">{{
